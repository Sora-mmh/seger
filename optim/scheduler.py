...
def get_scheduler(cfg, optimizer):
    ...
    if cfg.trainer.scheduler_kwargs is not None:
        ...
        if kwargs["warmup_iters"] > -1:
            ...
            if not use_iters:
                t_initial, warmup_t, decay_t, patience_t = [
                    t / cfg.data.train_size
                    for t in [t_initial, warmup_t, decay_t, patience_t]
                ]
        ...
        if name == "one_cycle":
            lr_scheduler = OneCycleLR(
                optimizer=optimizer,
                max_lr=cfg.lr,
                total_steps=(cfg.data.train_samples // cfg.batch_train)
                * cfg.epoch_full,
                # total_steps=cfg.epoch_full,
                pct_start=0.1,
                verbose=False,
            )
        elif name == "cosine_annealing":
            lr_scheduler = CosineAnnealingLR(
                optimizer=optimizer, T_max=cfg.data.train_samples // cfg.batch_train
            )
        elif name == "cyclic":
            lr_scheduler = CyclicLR(
                ...
                base_lr=cfg.lr,
                max_lr=1e-2,
                step_size_up=cfg.data.train_samples // cfg.batch_train,
                mode="triangular",
            )
        elif name == "cosine":
            lr_scheduler = CosineLRScheduler(
                **kwargs_common,
                t_initial=t_initial,
                cycle_mul=kwargs.get("lr_cycle_mul", 1.0),
