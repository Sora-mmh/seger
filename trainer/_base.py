...
from utils.metric import get_evaluator
...
class BaseTrainer:
    def __init__(self, cfg):
        ...
        log_msg(
            self.logger,
            "==> Using GPU: {} for Training".format(list(range(cfg.world_size))),
        )
        log_msg(self.logger, "==> Building model")
        self.net = get_model(cfg.model)
        self.net.to("cuda:{}".format(cfg.local_rank))
        # self.net.model.decoder.train()
        # for param in self.net.encoder.parameters():
        #     param.requires_grad = False

        self.net.encoder.train()
        self.net.decoder.train()
        self.net.seg_head.train()
        (
            log_msg(
                self.logger,
                f"==> Load checkpoint: {cfg.model.kwargs['checkpoint_path']}",
            )
            if cfg.model.kwargs["checkpoint_path"]
            else None
        )
        (
            print_networks(
                [self.net],
                torch.randn(
                    self.cfg.fvcore_b, self.cfg.fvcore_c, self.cfg.size, self.cfg.size
                ).cuda(),
                self.logger,
            )
        ...
        if cfg.dist and cfg.trainer.sync_BN != "none":
            ...
            syncbn_dict = {
                "apex": ApexSyncBN,
                "native": torch.nn.SyncBatchNorm.convert_sync_batchnorm,
                "timm": TIMMSyncBN,
            }
        ...
        self.loss_terms = get_loss_terms(
            cfg.loss.loss_terms, device="cuda:{}".format(cfg.local_rank)
        )
        ...
        # =========> dataset <=================================
        cfg.logdir_train, cfg.logdir_test = (
            f"{cfg.logdir}/show_train",
            f"{cfg.logdir}/show_test",
        )
        ...
        cfg.data.train_size, cfg.data.test_size = len(self.train_loader), len(
            self.test_loader
        )
        cfg.data.train_length, cfg.data.test_length = (
            self.train_loader.dataset.length,
            self.test_loader.dataset.length,
        )
        self.cls_names = self.cfg.data.cls_names
        self.mixup_fn = (
            Mixup(**cfg.trainer.mixup_kwargs)
            if self.cfg.trainer.mixup_kwargs is not None
            and cfg.trainer.mixup_kwargs["prob"] > 0
            else None
        )
        ...
        if cfg.trainer.resume_dir:
            state_dict = torch.load(
                cfg.model.kwargs["checkpoint_path"], map_location="cpu"
            )
    ...
    def _finish(self):
        ...
        f.write(msg)
        #############################################
        ############ Include max of metrics #########
        # for i in range(len(metric_list[0])):
        #     if i == 0:
        #         keys = list(self.metric_recorder.keys())
        #         keys = ["_".join(key.split("_")[:2]) for key in keys]
        #     else:
        #         keys = list(self.metric_recorder.keys())
        #         keys = ["_".join(key.split("_")[:2]) + " (Max)" for key in keys]
        #     col_w = max(len(key) for key in keys) + 2
        #     # header = "".join(metric.ljust(col_width) for metric in metrics)
        #     # data = "".join(f"{value:<{col_width}.2f}" for value in values)
        #     msg = ""
        #     header = ""
        #     for j in range(len(metric_list)):
        #         header += f"{keys[j].ljust(col_w)}"
        #         msg += f"{metric_list[j][i]:<{col_w}.3f}"
        #     header += "\n"
        #     f.write(header)
        #     msg += "\n\n"
        #     f.write(msg)
        ...
        while self.epoch < self.epoch_full and self.iter < self.iter_full:
            ...
            if self.iter % train_length == 0:
                ...
                (
                    ...
                    if hasattr(self.optim, "sync_lookahead")
                    else None
                )
                if (
                    self.epoch >= self.cfg.trainer.test_start_epoch
                    or self.epoch % self.cfg.trainer.test_per_epoch == 0
                ):
                    self.test()
                else:
                    self.test_ghost()
                ...
                total_time_str = str(
                    datetime.timedelta(seconds=int(self.cfg.total_time))
                )
                eta_time_str = str(
                    datetime.timedelta(
                        seconds=int(
                ...
                self.reset(isTrain=True)
                (
                    self.train_loader.sampler.set_epoch(int(self.epoch))
                    if self.cfg.dist
                    else None
                )
                train_loader = iter(self.train_loader)
    ...
    def save_checkpoint(self):
        if self.master:
            ...
            if self.scheduler:
                checkpoint_info["scheduler"] = (self.scheduler.state_dict(),)
            ...
            if self.epoch % self.cfg.trainer.test_per_epoch == 0:
                torch.save(
                    checkpoint_info["net"], f"{self.cfg.logdir}/net_{self.epoch}.pth"
                )
