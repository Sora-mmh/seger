...
def init_checkpoint(cfg):
    ...
    if cfg.trainer.resume_dir:
        ...
        if checkpoint_path == "":
            ...
            cfg.model.kwargs["checkpoint_path"] = "{}/{}".format(
                cfg.logdir, checkpoint_path.split("/")[-1]
            )
        ...
        if cfg.master:
            logdir_sub = (
                cfg.trainer.logdir_sub
                if cfg.trainer.logdir_sub != ""
                else time.strftime("%Y%m%d-%H%M%S")
            )
            # logdir_exp = '{}_{}_{}_{}'.format(cfg.trainer.name, cfg.model.name, cfg.data.type, logdir_sub)
            logdir_exp = f"{cfg.model.name.upper()[:-3]}_{cfg.trainer.name}_{cfg.cfg_path.replace('.', '_')}_{logdir_sub}"
            logdir = logdir_exp
            # cfg.trainer.run_experiment = (
            #     cfg.trainer.resume_dir if cfg.trainer.resume_dir != "" else logdir
            # )
            idx = 0
        ...
        if hasattr(cfg, "__dict__"):
            for key, val in cfg.__dict__.items():
                if not key.startswith("_"):
                    ret.update(
                        _parse_Namespace(val, "{}.{}".format(base_str, key).lstrip("."))
                    )
        else:
...
class ProgressMeter(object):
    ...
    def get_msg(self, iter, iter_full, epoch=None, epoch_full=None, prefix=None):
        entries = [
            self.iter_fmtstr_iter.format(
                prefix if prefix else self.default_prefix,
                iter / iter_full * 100,
                iter,
                iter_full,
                epoch,
