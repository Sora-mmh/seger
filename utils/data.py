...
def wait_for_path(path, check_interval=1, timeout=600):
    ...
    # prefix = (
    #     "/home/mmhamdi/workspace/unsupervised/Unsupervised-Anomlay-Detection/railad/"
    # )
    # if "anomalous" in path:
    #     path = prefix + path.split(prefix)[-1]
    while not os.path.exists(path):
        ...
        if elapsed_time >= timeout:
            ...
            return False
        print(
            f"Path does not exist: {path}. Checking again in {check_interval} seconds..."
        )
...
def get_scales(
    n_scale, base_h, base_w, min_h, max_h, min_w, max_w, check_scale_div_factor=32
):
