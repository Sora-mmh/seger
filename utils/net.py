...
from utils.util import log_msg
...
def init_training(cfg):
    ...
    if cfg.dist:
        ...
        torch.distributed.init_process_group(
            ...
            timeout=datetime.timedelta(seconds=7200 * 10),
        )
    ...
    if cfg.trainer.data.batch_size:
        cfg.trainer.data.batch_size_per_gpu = (
            cfg.trainer.data.batch_size // cfg.world_size
        )
        assert (
            cfg.trainer.data.batch_size_per_gpu * cfg.world_size
            == cfg.trainer.data.batch_size
        )
    else:
        cfg.trainer.data.batch_size = (
            cfg.trainer.data.batch_size_per_gpu * cfg.world_size
    ...
    if cfg.trainer.data.batch_size_test:
        cfg.trainer.data.batch_size_per_gpu_test = (
            cfg.trainer.data.batch_size_test // cfg.world_size
        )
        assert (
            cfg.trainer.data.batch_size_per_gpu_test * cfg.world_size
            == cfg.trainer.data.batch_size_test
        )
    else:
        cfg.trainer.data.batch_size_test = (
            cfg.trainer.data.batch_size_per_gpu_test * cfg.world_size
...
def reduce_tensor(tensor, world_size, mode="sum", sum_avg=True, rank=0):
    ...
    if mode == "sum":
        ...
        size[0] = world_size
        tensor_out = torch.zeros_like(
            tensor_, dtype=tensor_.dtype, device=tensor_.device
        )
