    ...
    def forward(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if (idx + 1) in self.teachers:
                ...
                H = int(np.sqrt(L))
                fea = (
                    fea.view(B, H, H, C).permute(0, 3, 1, 2).contiguous()
                )  # B, C, H, W
                out.append(fea)
...
class ViTSegHead(nn.Module):
    def __init__(
        ...
        super(ViTSegHead, self).__init__()
...
class ViTSeg(nn.Module):
    def __init__(self, encoder):
        ...
        self.decoder = UNetDecoder([384] * 3)
        self.seg_head = ViTSegHead(self.decoder.decoder_channels[-1])
...
@MODEL.register_module
def vitseg(pretrained=False, **kwargs):
    model = ViTSeg(**kwargs)
    return model
...
if __name__ == "__main__":
    ...
    from utils.util import get_timepc, get_net_params
    ...
    encoder = _Namespace()
    encoder.name = "vit_small_patch16_224_dino"
    encoder.kwargs = dict(
        pretrained=False,
        checkpoint_path="",
        strict=True,
    ...
    model.name = "vitseg"
    model.kwargs = dict(
        pretrained=False, checkpoint_path="", strict=True, encoder=encoder
    )

    net = vitseg(encoder=encoder).cuda()
    net.train()
    y = net(x)
    ...
    with torch.no_grad():
        ...
        t_e = get_timepc()
    print(
        "[GFLOPs: {:>6.3f}G]\t[Params: {:>6.3f}M]\t[Speed: {:>7.3f}]\n".format(
