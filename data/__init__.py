...
def get_dataset(cfg):
    train_transforms = get_transforms(
        cfg, train=True, cfg_transforms=cfg.data.train_transforms
    )
    test_transforms = get_transforms(
        cfg, train=False, cfg_transforms=cfg.data.test_transforms
    )
    gt_train_transforms = get_transforms(
        cfg, train=True, cfg_transforms=cfg.data.target_transforms
    )
    gt_test_transforms = get_transforms(
        cfg, train=False, cfg_transforms=cfg.data.target_transforms
    )
    train_set = DATA.get_module(cfg.data.type)(
        cfg,
        train=True,
        transform=train_transforms,
        target_transform=gt_train_transforms,
    )
    test_set = DATA.get_module(cfg.data.type)(
        cfg, train=False, transform=test_transforms, target_transform=gt_test_transforms
    )
    ...
    if cfg.dist:
        if cfg.data.sampler == "naive":
            ...
            raise NotImplementedError(
                "sampler '{}' is not implemented".format(cfg.data.sampler)
            )
